...
class DeploymentTester(object):
	...
	def validate(self):
		...
		for back in self.backend_containers:
			print(back)
		print("")
		print("Encountered %s reply texts:" % (len(self.replies)))
		...
		result += self.check_set_len(self.replies, "replies", [NUM_FRONTEND_CONTAINERS * NUM_BACKEND_CONTAINERS, NUM_FRONTEND_CONTAINERS * NUM_BACKEND_CONTAINERS / 2])
		if result == 0:
